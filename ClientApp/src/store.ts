...
export default new Vuex.Store({
  ...
  mutations: {
    elegirTambo(state, index) {
      ...
      const tambo = state.tambos[index].nombre;
      state.tamboElegido = tambo;
    },
    elegirEvento(state, index) {
      const evento = state.eventos[index].descripcion;
      state.eventoElegido = evento;
    },
    elegirTipoEvento(state, index) {
      const tipoEvento = state.tiposEventos[index].descripcion;
      state.tipoEventoElegido = tipoEvento;
    },
    elegirRp(state, index) {
      const rp = state.listaRp[index].rp;
      state.rpElegido = rp;
    },
    elegirParticipante(state, index) {
      const part = state.participantes[index].nombre;
      state.particElegido = part;
    },
    elegirTratamiento(state, index) {
      const tratam = state.tratamientos[index].nombre;
